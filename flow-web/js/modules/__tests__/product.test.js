...
import CollapseUI from '@app/public/js/modules/collapse';
import ProductUI from '@app/public/js/modules/product';

jest.mock('@app/public/js/modules/collapse');
...
describe('Product UI class', () => {
  ...
  describe('instantiates without sub UI components', () => {
    ...
    it("doesn't instantiate sub UI components without config", () => {
      expect(productUI.collapse).toBe(false);
    });
  ...
  describe('instantiates with sub UI components', () => {
    it('builds a collapse instance if needed', () => {
      const productUI = new ProductUI(productEl, { initialLinesToShow: 4 });
      expect(productUI.collapse).toBeInstanceOf(CollapseUI);
    });

    it('initialises correctly', () => {
      const productUI = new ProductUI(productEl, {
        initialLinesToShow: 4,
      });

      productUI.init();

      expect(productUI.collapse.init).toBeCalledTimes(1);
    });
    ...
    it('cleans up correctly', () => {
      ...
      expect(productUI.collapse.cleanup).toBeCalledTimes(1);
    });
  });
