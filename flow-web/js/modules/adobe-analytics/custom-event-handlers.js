...
export const adobeEventHandlers = {
  ...
  /**
   * @todo remove when removing COOKIE_FEAT_NO_AMP_PRODUCTS as this binds to the
   * RecommendedRetailer Buy Now button
   */
  product_buy_now_click: ({ name, price }) => {
    track(
      'Product Body Buy Link Click',
      {
        [adobeVars.LINK_LABEL]: 'Product Body - Buy Link Click',
        [adobeProps.LINK_LABEL]: makeEVarLink(adobeVars.LINK_LABEL),
