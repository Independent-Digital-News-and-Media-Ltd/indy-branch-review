...
const initLightbox = () => {
  if (!JSGlobals.featureFlags.feat__no_amp) {
    const loginBtns = [
...
export const removeBookmarkLocalStorage = (articleId) => {
...
export const deleteBookmark = async (articleId, button) => {
...
export const submitBookmark = async (articleId, button) => {
  if (button.classList.contains('submitting')) {
    return;
  }
  button.classList.add('submitting');
...
export const updateBookmarkState = (...args) => {
  const [, button] = args;
