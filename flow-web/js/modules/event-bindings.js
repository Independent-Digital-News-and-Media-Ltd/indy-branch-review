...
import {
  ...
  dispatchProductCarouselReadReviewClick,
  dispatchProductCarouselBuyNowClick,
  dispatchProductComparisonCheckboxClick,
  dispatchProductComparisonModalOpen,
  dispatchProductComparisonReadReviewClick,
  dispatchProductComparisonBuyNowClick,
...
/**
 * @todo remove when removing COOKIE_FEAT_NO_AMP_PRODUCTS as this bind events to
 * the ProductCarousel elements
 */

export const eventProductCarouselLoad = () => {
  ...
  productComparison?.addEventListener('click', (event) => {
    if (event.target.closest(`.${CLASS_JS_PRODUCT_COMPARISON_REVIEW_BUTTON}`)) {
      ...
      dispatchProductComparisonCheckboxClick();
    } else if (event.target.closest(`.${CLASS_JS_PRODUCT_COMPARISON_BUTTON}`)) {
      dispatchProductComparisonModalOpen();
    } else if (
      event.target.closest(
...
export const run = () => {
  ...
  thisModule.eventProductCarouselLoad();
