...
import { ID_JW_LOADING_IMAGE } from '@app/constants/ids';
...
export const removePosterImage = () => {
  document.getElementById(ID_JW_LOADING_IMAGE)?.remove();
};
...
export const permutiveEnabled = (consent) =>
  !!(consent && window.JSGlobals.permutive);
...
export const setInitialParamsToAdTag = (adTag) => {
  ...
  adTag.custParams.set('adpos', thisModule.getAdPos());
};

/**
 * @returns {import('./AdTag/AdTagURL').AdTagURL} adTag
 */
export const createAdTag = () => {
  const adTag = new AdTagURL('https://pubads.g.doubleclick.net/gampad/ads');
