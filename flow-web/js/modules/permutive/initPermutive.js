...
export const identifyPianoUser = () => {
  identifyUser();

  const birthYear = getCookie(COOKIE_BIRTH_YEAR);
  const pianoId = getCookie(COOKIE_PUID);
  const userStatus = getCookie(COOKIE_USER_STATUS); // premium or registered

  permutive.track('PianoProfile', {
    birth_year: parseInt(birthYear),
    piano_user_status: userStatus || 'anonymous',
    piano_id: pianoId,
  });
};
...
// on which we catch and set permutiveRejected to true to indicate dependent
// modules (like apvSegment) that it has been thrown in permutiveReady callback
