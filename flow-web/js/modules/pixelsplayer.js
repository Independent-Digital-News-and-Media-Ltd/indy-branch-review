...
import {
  prepareParams as iasPrepareParams,
  getParams as iasGetParams,
} from './ias';
...
const addDugoutSegmentToAdUnit = (adUnit) => {
  if (!JSGlobals.article?.isDugoutHero) {
    return adUnit;
  }
  const adParts = adUnit.split('/');
  const lastSegment = adParts[adParts.length - 3];
  adParts.splice(adParts.length - 3, 1, lastSegment);
  return adParts.join('/');
};

const removeDugoutReferences = (adTag) => {
  if (!JSGlobals.article?.isDugoutHero) {
    return adTag;
  }
  try {
...
const enrichTag = (containerId, adTag, mediaid, playerType) => {
  ...
  // ias params
  customParams += `&${iasGetParams(containerId)}`;
  ...
  try {
    ...
    adTag += `&gdpr=${gdpr}`;
    if (gdpr === '1') {
      adTag += `&gdpr_consent=${getCookie('euconsent-v2', 'none')}`;
    ...
    if (replacementAdTagUnit) {
      adTag = adTag.replace(
        /(iu=).*?(&)/,
        `$1${addDugoutSegmentToAdUnit(replacementAdTagUnit)}$2`,
      );
    }
  } catch (e) {
    // do nothing if the adTag cannot be parsed
...
export default async (consent, prebidEnabled) => {
  permutiveEnabled = consent && window.JSGlobals.permutive;
  ...
  if (typeof pixels === 'function') {
    ...
    for (let i = 0; i < scripts.length; i++) {
      if (scripts[i] && scripts[i].dataset.pixelsPlayerUid) {
        const containerId = scripts[i].dataset.pixelsPlayerUid;
        const data = JSON.parse(scripts[i].dataset.pixelsPlayerData);
